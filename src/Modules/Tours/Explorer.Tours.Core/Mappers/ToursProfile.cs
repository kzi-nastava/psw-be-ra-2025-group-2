...
public class ToursProfile : Profile
{
    public ToursProfile()
    {
        ...
        CreateMap<TouristObjectDto, TouristObject>()
            .ForMember(dest => dest.Category, opt => opt.MapFrom(src => Enum.Parse<TouristObjectCategory>(src.Category)));
    ...
    .ForMember(dest => dest.ActivityTypes,
        ...
        CreateMap<TourDurationDto, TourDuration>()
            .ConstructUsing(src => new TourDuration((TransportType)src.TransportType, src.Minutes));

        CreateMap<TourDuration, TourDurationDto>()
            .ForMember(dest => dest.TransportType, opt => opt.MapFrom(src => (TransportTypeDto)src.TransportType));

        CreateMap<TourReviewDto, TourReview>().ReverseMap();
        CreateMap<TourReview, TourReviewPublicDto>()
            .ForMember(d => d.TouristName, o => o.Ignore());
        ...
        CreateMap<AverageCostDto, AverageCost>()
     .ConstructUsing(src =>
         AverageCost.Create(
             src.Currency,
             AverageCostBreakdown.Create(
                 (src.Breakdown ?? new AverageCostBreakdownDto()).Tickets,
                 (src.Breakdown ?? new AverageCostBreakdownDto()).Transport,
                 (src.Breakdown ?? new AverageCostBreakdownDto()).FoodAndDrink,
                 (src.Breakdown ?? new AverageCostBreakdownDto()).Other
             ),
             src.Disclaimer
         )
     );
        ...
        CreateMap<AverageCost, AverageCostDto>()
            .ForMember(dest => dest.Breakdown, opt => opt.MapFrom(src => src.Breakdown));

        CreateMap<AverageCostBreakdown, AverageCostBreakdownDto>().ReverseMap();
    }
}
